...
module.exports = function Extract (moduleOptions) {
  ...
  this.nuxt.hook('generate:routeCreated', async ({ route }) => {
    ...
    await processPayload(statePath)
    return await processPayload(payloadPath)
  })

  /**
   * Converts regex matches for both PAGE and PAYLOAD responses into an Array of URL Objects
   */
  function urlsFromMatches (matches, isPayloadFormat = false) {
    const responsiveImagesRegex = isPayloadFormat ? /,?(?:\s|%20){1}[\d.]+[xwh](?:\s|%20)?,?(?:\s|%20)?/gm : /,?\s{1}[\d.]+[xwh]\s?,?\s?/gm
    const urls = []
    for (const match of matches) {
      const url = isPayloadFormat ? new URL(decodeURIComponent(JSON.parse('"' + removeTrailingBackslash(match[1]) + '"'))) : match[1]

      let matchURLStrings = [url.toString()]
      const responsiveImageMatches = url.toString().match(responsiveImagesRegex)
      if (responsiveImageMatches && responsiveImageMatches.length) {
        matchURLStrings = url.toString().split(responsiveImagesRegex).filter(item => item.length)
      }
  ...
  async function downloadAndReplacePayloadImageLinks (payload, urls) {
    ...
    await Promise.all(
      urls.map(async (url) => {
        ...
        if (options.downloadPayloadMedia) {
          const imgPath = join(baseDir, name)
